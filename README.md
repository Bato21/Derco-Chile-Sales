...
This repository holds a complete BI project built on 14 years of DERCO Chile retail vehicle sales
(550,033 transactions, May 2009 to March 2022). It contains one executable notebook that runs the whole
pipeline, the analysis-ready dataset that notebook produces, a 13-slide stakeholder deck in three formats,
and the documentation needed to reproduce every number.
...
`BI_FinalProject_Team1/` is the folder handed in for grading. It is flat and self-contained:

| File | What it is |
|---|---|
| `BI2026_FinalProject.ipynb` | The final notebook, with all outputs saved. Stages 1 to 5 plus the AI-use disclosure. |
| `derco_sales_clean.parquet` | The full analysis-ready dataset, 550,032 rows by 15 columns, 51 MB on disk. |
| `BI2026_FinalProject_DERCO.pdf` | The presentation, 13 pages at 297 × 167 mm (16:9). |
| `README.md` | The same documentation you are reading, adapted to the flat folder. |

Those four files are copies. The originals live at `notebooks/BI2026_FinalProject.ipynb`,
`data/derco_sales_clean.parquet`, `presentation/BI2026_FinalProject_DERCO.pdf` and this file. If you change
an original, copy it across again; nothing syncs automatically.

The raw source workbook is deliberately not in the bundle. It is course-provided material that is not to be
redistributed, and the clean Parquet already carries every column the analysis uses.
...
from the flat submission folder, or from Google Colab once the `.xlsx` has been uploaded. It prints the two
folders it settled on in the setup cell, so a wrong guess is visible immediately rather than three cells later.
...
The deck exists in three formats built from one source of numbers.
