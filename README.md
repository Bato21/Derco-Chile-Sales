...
`BI_FinalProject_Team1/` is the folder handed in. Four files:

| File | What it is |
|---|---|
| `BI2026_FinalProject.ipynb` | The analysis. Five BI stages end to end, with all outputs saved. |
| `derco_sales_clean.parquet` | The analysis-ready dataset the notebook produces: 550,032 rows, 15 columns, 51 MB. |
| `BI2026_FinalProject_DERCO.pdf` | The 13-slide presentation of the findings. |
| `README.md` | This document. |

They are copies of `notebooks/BI2026_FinalProject.ipynb`, `data/derco_sales_clean.parquet`,
`presentation/BI2026_FinalProject_DERCO.pdf` and this file. The raw source workbook is not included: it is
course-provided material that is not redistributed, and the clean table carries every column the analysis uses.

## The data

Provided by the course as the exam file for this project. DERCO Chile retail vehicle sales, 5 May 2009 to
28 March 2022. Full provenance in `data/DATA_SOURCE.md`.
...
Parquet is used rather than CSV because it preserves column types and is less than half the size.
`data/derco_sales_sample.csv` is a 2,000-row seeded sample for inspection in a spreadsheet.
...
source workbook, so it runs unchanged from `notebooks/`, from the repository root, from the submission folder, or
from Colab once the workbook is uploaded. It prints the folders it resolved in the setup cell.
